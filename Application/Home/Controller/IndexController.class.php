...
class IndexController extends Controller
{
    ...
    public function ptqrShow()
    {
        ...
        $time = $microTime[1] . ceil($microTime[0] * 1000);
        $url = "http://ptlogin2.qq.com/ptqrshow?appid=549000912&e=2&l=M&s=3&d=72&v=4&t=0.".$time."&daid=5";
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_REFERER, '');
        curl_setopt($ch, CURLOPT_COOKIEJAR, $this->tdCode); // 发送初始Cookie
    ...
    public function tdCodeCheck()
    {
        ...
        $microTime = explode(" ", microtime());
        $time = $microTime[1].ceil($microTime[0] * 1000);
        $url = "http://ptlogin2.qq.com/ptqrlogin?u1=http%3A%2F%2Fqzs.qq.com%2Fqzone%2Fv5%2Floginsucc.html%3Fpara%3Dizone&from_ui=1&ptlang=2052&action=0-0-".$time."&js_ver=10176&js_type=1&login_sig=&pt_uistyle=40&aid=549000912&daid=5";
        $curl = curl_init($url);
        $headers[] = 'Host:ptlogin2.qq.com';
        $headers[] = 'Accept：*/*';
        ...
        if (strpos($result, "'66'")) {
            ...
            return jsonObject(array("status" => 5, "msg" => "接口已变更"));
        }
    }
